...
class FECDevice(AntPlusDevice):
    ...
    def __init__(self, node, device_number, device_type, transmission_type):
        if device_type != FECDevice.FEC_DEVICE_TYPE:
            ...
            return
        channel_period = (
            FECDevice.FEC_CHANNEL_PERIOD
        )  # device profile standard # TODO : cleanup magic numbers
    ...
    def on_antplus_bcdata(self, data):
        ...
        if data[0] == FECDevice.Page.GENERAL_FE_DATA:
            self.info["equipment_type"] = (
                data[1] & 0xF
            )  # store with .info (static data)
            self.info["p16_capabilities"] = (
                data[7] & 0xF
            )  # store with .info (static data)
            # elapsed_time (in s)
            ...
            self.status["distance_traveled"] = (
                self.status["distance_traveled"] & ~0xFF
            ) + data[3]
            ...
            fe_state = data[7] >> 4
            self.status["fe_state"] = (
                fe_state & 0x7
            )  # 0=reserved, 1=asleep, 2=ready, 3=in_use, 4=finished/paused
            ...
            if new_event_count != prev_event_count:
                instant_power = int.from_bytes(data[5:7], byteorder="little") & 0xFFF
                if instant_power != 0xFFF:  # has valid power data
                    bikedata["instant_power"] = instant_power  # in Watt
                    prev_total_power = bikedata["total_power"]
                    new_total_power_bytes = int.from_bytes(
                        data[3:5], byteorder="little"
                    )  # in Watt
                    if new_total_power_bytes < prev_total_power & 0xFFFF:  # rollover
                        bikedata["total_power"] += 0xFFFF
                    bikedata["total_power"] = (
                        bikedata["total_power"] & ~0xFFFF
                    ) + new_total_power_bytes
                    bikedata["avg_power"] = (
                        bikedata["total_power"] - prev_total_power
                    ) / (new_event_count - prev_event_count)
                if data[2] != 0xFF:
                    bikedata["instant_cadence"] = data[2]  # in rpm
                bikedata["trainer_status"] = (
                    data[6] >> 4
                ) & 0xF  # calibration required / user config required
                ...
                lap_toggle_bit = (fe_state >> 3) & 0x1
                if lap_toggle_bit != self.status.get(
                    "lap_toggle_bit", 0
                ):  # new lap event
                    self.status["lap_events"] += 1
                    self.status["lap_toggle_bit"] = lap_toggle_bit
            ...
            if max_resistance != 0xFFFF:
                self.info["maximum_resistance"] = int.from_bytes(
                    data[5:7], byteorder="little"
                )
            ...
            zero_offset_calibration_status = (status >> 6) & 0x1
            spindown_calibration_status = (status >> 7) & 0x1
            progress["zero_offset_calibration_status_txt"] = (
                "Zero Offset Calibration :"
                + status_txts[zero_offset_calibration_status]
            )
            progress["spindown_calibration_status_txt"] = (
                "Spin-down Calibration :" + status_txts[spindown_calibration_status]
            )
            # conditions
            conditions = data[2]
            ...
            progress["temperature_condition_txt"] = (
                "Temperature : " + condition_txts[temperature_condition]
            )
            progress["speed_condition_txt"] = (
                "Speed : " + condition_txts[speed_condition]
            )

            progress["temperature"] = data[3]
            progress["target_speed"] = int.from_bytes(data[4:6], byteorder="little")
            progress["target_spindown_time"] = int.from_bytes(
    ...
    def set_resistance(self, resistance):
        ...
        if resistance_value >= 0 and resistance_value <= 200:
            self.status[
                "set_resistance"
            ] = resistance_value  # store so we can monitor later
            _reserved = 0xFF
            req_data = array.array(
                "B",
                [
                    FECDevice.Page.CONTROL_BASIC_RESISTANCE,
                    _reserved,
                    _reserved,
                    _reserved,
                    _reserved,
            ...
            _logger.info(f"FEC:set_resistance, send data : {req_data}")
    ...
    def get_target_power(self):
        ...
        if (
            cmd_status["last_cmd_id"] == FECDevice.Page.CONTROL_TARGET_POWER
            and cmd_status["cmd_status"] == 0
        ):  # 0 = Pass
    ...
    def set_target_power(self, target_power):
        if target_power >= 0.0 and target_power <= 4000.0:
            self.status[
                "set_target_power"
            ] = target_power  # store so we can monitor later
            target_power_value = int(target_power * 4.0) & 0xFFFF
            _reserved = 0xFF
            req_data = array.array(
                "B",
                [
                    FECDevice.Page.CONTROL_TARGET_POWER,
                    _reserved,
                    _reserved,
                    _reserved,
                    _reserved,
            ...
            self.channel.send_acknowledged_data(req_data)  # sync by channel.py
            _logger.info(f"FEC:set_target_power, send data : {req_data}")

    # use the units from the device profile
    def set_wind_resistance(
        self, wind_speed=0xFF, wind_resistance_coeff=0xFF, drafting_factor=0xFF
    ):
        _reserved = 0xFF
        req_data = array.array(
            "B",
            [
                FECDevice.Page.CONTROL_WIND_RESISTANCE,
                _reserved,
                _reserved,
                _reserved,
                _reserved,
                wind_resistance_coeff,
                wind_speed,
                drafting_factor,
            ],
        )
        self.channel.send_acknowledged_data(req_data)  # sync by channel.py
        _logger.info(f"FEC:set_wind_resistance, send data : {req_data}")
    ...
    def set_track_resistance(self, grade=None, rolling_resistance_coeff=None):
        ...
        req_data = array.array(
            "B",
            [
                FECDevice.Page.CONTROL_TRACK_RESISTANCE,
                _reserved,
                _reserved,
                _reserved,
                _reserved,
            ]
            + list(grade_value.to_bytes(2, byteorder="little"))
            + [rr_coeff_value],
        )
        self.channel.send_acknowledged_data(req_data)  # sync by channel.py
        _logger.info(f"FEC:set_track_resistance, send data : {req_data}")
    ...
    def set_user_config(
        self, user_weight=None, bicycle_weight=None, wheel_diam=None, gear_ratio=0.0
    ):
        _reserved = 0xFF
        ...
        req_data = array.array(
            "B",
            [FECDevice.Page.USER_CONFIG]
            + list(user_weight_value.to_bytes(2, byteorder="little"))
            + [
                _reserved,
                wheel_diam_mm + ((bicycle_weight_value & 0xF) << 4),
                bicycle_weight_value >> 4,
                wheel_diam_cm,
                gear_ratio_value,
            ],
        )
        self.channel.send_acknowledged_data(req_data)  # sync by channel.py
        _logger.info(f"FEC:set_user_config, send data : {req_data}")
    ...
    def start_calibration(self, calibration_type=None, status_callback=None):
        ...
        calibration_type = (
            calibration_type or FECDevice.CalibrationType.ZERO_OFFSET_CALIBRATION
        )
        _reserved = 0x0
        _unused = 0xFF
        if status_callback:
        ...
        req_data = array.array(
            "B",
            [
                FECDevice.Page.CALIBRATION_REQUEST_RESPONSE,
                calibration_type,
                _reserved,
                _unused,
                _unused,
        ...
        self.channel.send_acknowledged_data(req_data)  # sync by channel.py
        _logger.info(f"FEC:start_calibration:sending request data : {req_data}")
    ...
    def stop_calibration(self):
        ...
        req_data = array.array(
            ...
            [
                FECDevice.Page.CALIBRATION_REQUEST_RESPONSE,
                FECDevice.CalibrationType.CANCEL,
                _reserved,
                _unused,
                _unused,
                _unused,
    ...
    def _calibration_status_callback(self, status):
        ...
        _logger.info(f"FEC:calibration status : {status}")
