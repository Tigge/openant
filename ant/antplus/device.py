...
import logging
import struct # TODO : what's this??
import threading
import sys
...
class AntPlusDevice():
    ...
    def __init__(self, node, device_number, device_type, transmission_type, channel_period):
        ...
        self.channel.set_period(self.channel_period) #std waarde uit de <> std profiles
    ...
    def on_antplus_bcdata(self, data):
        if data[0] == AntPlusDevice.Page.MANUFACTURER_INFO : # manufacturer's information
            ...
            self.info['software_revision'] = (100.0*data[3] + supplimental_sw_revision) / 1000.0 #float
    ...
    def _request_data_page_ack(self, page_num, descriptor_bytes = [0xFF, 0xFF]):
        ...
        _unused = 0xFF
        req_data = array.array('B',[AntPlusDevice.Page.REQUEST_DATA,_unused,_unused,
                                    descriptor_bytes[0],descriptor_bytes[1],transmit_until_ack,page_num, command_type])
        _logger.info(f"sending request data : {req_data}")
        self.status['print_cnt'] = 20 # let's print the next 20 bc msgs
        self.channel.send_acknowledged_data(req_data) # dit gaat blocken tot EVENT_TRANSFER_TX_COMPLETED
    ...
    def _request_data_page(self, page_num, descriptor_bytes = [0xFF, 0xFF]):
        ...
        _unused = 0xFF
        req_data = array.array('B',[AntPlusDevice.Page.REQUEST_DATA,_unused,_unused,
                                    descriptor_bytes[0],descriptor_bytes[1],number_of_transmits,page_num, command_type])
        _logger.info(f"sending request data : {req_data}")
        self.status['print_cnt'] = 20 # let's print the next 20 bc msgs
        try:
            self.channel.send_acknowledged_data(req_data) # dit gaat blocken tot EVENT_TRANSFER_TX_COMPLETED
        except:
            # hoe kunnen we de specifieke AntException opvangen??
    ...
    def get_manufacturer_info(self):
        ...
        return {'hardware_revision' : self.info['hardware_revision'],
    ...
    def get_product_info(self):
        ...
        # the data are now stored under self.info        
        return {'serial_number' : self.info['serial_number'],
