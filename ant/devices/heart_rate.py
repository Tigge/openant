...
class HeartRate(AntPlusDevice):
    ...
    def __init__(self, node: Node, device_id:int=0, name:str="heart_rate", trans_type:int=0):
        super().__init__(node, device_type=DeviceType.HeartRate.value, device_id=device_id, period=8070, name=name, trans_type=trans_type)
    ...
    def on_data(self, data):
        ...
        if (page & 0x0F) <= 7:
            ...
            self.data['heart_rate'].page_specific = int.from_bytes(data[1:4], byteorder='little')
            self.data['heart_rate'].beat_time = int.from_bytes(data[4:6], byteorder='little') / 1024
            self.data['heart_rate'].beat_count = data[6]
            self.data['heart_rate'].heart_rate = data[7]

            if (dp == 0x01):
                self.data['heart_rate'].operating_time = self.data['heart_rate'].page_specific * 2
            elif (dp == 0x02):
                self.data['heart_rate'].manufacturer_id_lsb = data[1]
                self.data['heart_rate'].serial_number = int.from_bytes(data[2:4], byteorder='little')
            # background page product info
            elif (dp == 0x03):
                pass
            # main page previous heart beat
            elif (dp == 0x04):
                self.data['heart_rate'].previous_heart_beat_time = int.from_bytes(data[2:4], byteorder='little') / 1024
            # swim interval stuff
            elif (dp == 0x05):
                pass
            elif (dp == 0x06):
                ...
                self.data['common'].last_battery_data.status = BatteryStatus((data[3] & 0x70) >> 4)
