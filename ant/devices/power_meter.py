    ...
    def on_data(self, data):
        ...
        if page == 0x10:
            ...
            if data[2] & (1 << 7) and data[2] != 0xFF:
                percent = data[2] ^ (1 << 7)
                self.data['power'].right_power = int((self.data['power'].instantaneous_power * percent) / 100)
                self.data['power'].left_power = self.data['power'].instantaneous_power - self.data['power'].right_power
        ...
        # standard torque
            ...
            delta_crank_period = (self._crank_period[1] + 65536 - self._crank_period[0] % 65536)
            ...
            if delta_update_count:
                ...
                if delta_crank_period:
                    self.data['power'].angular_velocity = (2 * math.pi * delta_update_count) / (delta_crank_period / 2048)
                else:
