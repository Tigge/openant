...
class AntSendDemo:
    ...
    def Create_Next_DataPage(self):
        ...
        self.DistanceAccu+=self.DistanceBetween # Add Distance beetween 2 ANT+ Ticks to Accumulated Distance
        ...
        self.TimeRollover_H = int(self.TimeRollover); # only integer
        ...
        if (self.ANTMessageCount < 3):
            ...
            self.ANTMessagePayload[4] = self.DistanceLow_HEX * 16 + self.var_speed_ms_H # Instaneus Speed, Note: INTEGER
            self.ANTMessagePayload[5] = self.var_speed_ms_L_HEX # Instaneus Speed, Fractional
    ...
    def on_event_tx(self, data):
        ...
        self.channel.send_broadcast_data(self.ANTMessagePayload) # Final call for broadcasting data
        print (self.ActualTime,"TX:",Device_Number,",",Device_Type,":", format_list(ANTMessagePayload))
    ...
    def OpenChannel(self):
        ...
        self.channel = self.node.new_channel(Channel.Type.BIDIRECTIONAL_TRANSMIT,0x00,0x00) # Set Channel, Master TX
        self.channel.set_id(Device_Number, Device_Type, 5) # set channel id as <Device Number, Device Type, Transmission Type>
...
def main():
    ...
    logging.basicConfig(filename='example.log',level=logging.DEBUG) # just for Debugging purpose, outcomment this in live version
